  ...
  for (i = 0; i < sizeM; i += 4)
  {
    ...
    for (j = 0; j < sizeN; j += 16)
    {
      ...
      for (int k = 0; k < sizeK; k++)
      {
        ...
        b2 = vld1q_f32(ptr_packing_b + 8);
        b3 = vld1q_f32(ptr_packing_b + 12);
...
{
    float *tosrc1, *tosrc2, *tosrc3, *tosrc4, *tosrc5, *tosrc6, *tosrc7, *tosrc8, *todst = a_block;
    int rows, cols;
...
void pack_b(float *b, float *b_block, const uint64_t n3)
{
    ...
    for (auto count = 0; count < N_BLOCKING; count += 16)
    {
        tosrc = b + count;
        for (auto count_second = 0; count_second < K_BLOCKING; count_second++)
        {
            ...
            vst1_f32_x4(todst + 8, vld1_f32_x4(tosrc + 8));
            vst1_f32_x4(todst + 12, vld1_f32_x4(tosrc + 12));
            tosrc += n3;
            todst += 16;
        }
    }
...
void mul(float *A, float *B, float *C, const uint64_t M, const uint64_t N, const uint64_t K)
{
    ...
    #pragma omp for private(m_count, n_count, k_count)
